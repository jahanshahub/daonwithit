...
// Part: OpenZeppelin/openzeppelin-contracts@4.5.0/Address
...
// Part: OpenZeppelin/openzeppelin-contracts@4.5.0/Context
...
// Part: OpenZeppelin/openzeppelin-contracts@4.5.0/IERC165
...
// Part: OpenZeppelin/openzeppelin-contracts@4.5.0/IERC20
...
// Part: OpenZeppelin/openzeppelin-contracts@4.5.0/ReentrancyGuard
...
// Part: OpenZeppelin/openzeppelin-contracts@4.5.0/IERC721
...
// Part: OpenZeppelin/openzeppelin-contracts@4.5.0/Ownable
...
// Part: OpenZeppelin/openzeppelin-contracts@4.5.0/SafeERC20
...
contract ERC721Staking is Ownable, ReentrancyGuard {
    ...
    function withdrawArray(uint256[] calldata _tokenIds) external nonReentrant {
        require(
    ...
    function _withdraw(
        ...
        require(stakerAddress[_tokenId] == user);
        uint index = 0;
        for (uint i = 0; i < stakedIDs[user].length; i++) {
            if(stakedIDs[user][i] == _tokenId) {
                index = i;
            }
        }
        for (uint i = index; i<stakedIDs[user].length-1; i++){
            stakedIDs[user][i] = stakedIDs[user][i+1];
        }
        stakedIDs[user].pop();

        stakerAddress[_tokenId] = address(0);
        nftCollection.transferFrom(address(this), user, _tokenId);
    }
    ...
    /// @dev Get the tokens staked by a staker
    function getStakedTokens(address _staker) external view returns (uint256[] memory tokenIds) {
        return stakedIDs[_staker];
    }
}// SPDX-License-Identifier: MIT
