...
export default function NFT() {
  ...
  async function callApi() {
    ...
    vaultcontract = new web3.eth.Contract(VAULTABI, STAKINGCONTRACT);
		...
		const nfts = await axios.get((moralisapi + `/nft/${NFTCONTRACT}/owners?chain=goerli&format=decimal`), {headers: config})
    .then(output => {
    ...
    const stakednfts = await vaultcontract.methods.userStakeInfo(account).call()
    .then(id => {
      return id;
    })
                  ...
                  {apicall.map((nft, i) => {
                    ...
                    {nftstk.map((nft, i) => {
                      async function unstakeit() {
                        vaultcontract.methods.withdraw([nft.tokenId]).send({ from: account });
                      }
                      return (
                        <div>
